<script>
	...
	function setupControls() {
		pane = createPane('Spiral Galaxy Controls');

		const galaxyFolder = createFolder(pane, 'Galaxy', true);
		galaxyFolder.addBinding(params, 'particleCount', { min: 50, max: 500, step: 10 });
		galaxyFolder.addBinding(params, 'maxRadius', { min: 30, max: 120, step: 5 });
		galaxyFolder.addBinding(params, 'spiralArms', { min: 2, max: 6, step: 1 });

		const motionFolder = createFolder(pane, 'Motion', true);
		motionFolder.addBinding(params, 'rotationSpeed', { min: 0.01, max: 0.5, step: 0.01 });
