<script>
	...
	function setupControls() {
		pane = createPane('Pendulum Wave Controls');

		const pendulumFolder = createFolder(pane, 'Pendulums', true);
		pendulumFolder.addBinding(params, 'pendulumCount', { min: 5, max: 25, step: 1 });
		pendulumFolder.addBinding(params, 'baseFrequency', { min: 0.1, max: 2.0, step: 0.05 });
		pendulumFolder.addBinding(params, 'pendulumLength', { min: 40, max: 120, step: 5 });
		pendulumFolder.addBinding(params, 'maxAngle', { min: 5, max: 45, step: 1 });
	...
	function draw() {
		...
		const angle = Math.sin(time * params.baseFrequency * Math.PI) * maxAngleRad;
