<script>
	...
	function setupControls() {
		pane = createPane('Pulsing Grid Controls');
	...
	function draw() {
		...
		const breathingFactor = Math.sin(time * params.breathingSpeed) * 0.2 + 1.0;
		...
		for (let row = 0; row < params.gridSize; row++) {
			for (let col = 0; col < params.gridSize; col++) {
				...
				// Radial wave effect
				const radialPhase = (time * params.waveSpeed - normalizedDistance) % 1;
				...
				const size = Math.min(baseSize * pulseFactor, params.maxDotSize);
